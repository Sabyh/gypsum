...
exports.objectUtil = {
    ...
    fromPath(obj, path, value, inject) {
    ...
    getValue(obj, path) { return this.fromPath(obj, path); },
    setValue(obj, path, value) { return this.fromPath(obj, path, value); },
    injectValue(obj, path, value) { return this.fromPath(obj, path, value, true); },
    hasProperty(src, path) {
        let parts = path.split('.');
        if (parts.length === 1)
    ...
    parse(queryString) {
        ...
        let temp = { value: '', prop: '' };
        let context = type === 'array' ? 'value' : 'prop';
        let level = 0;
        ...
        if (type === 'array')
            result = result.map((prop) => this.parse(prop));
        else
