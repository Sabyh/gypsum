...
import { API_TYPES, RESPONSE_CODES, RESPONSE_DOMAINS, Response, ResponseError, IResponseError, IService, IHookOptions } from './types';
    ...
    if (this.apiType === API_TYPES.REST && this._res) {
      ...
      if (!this._response.success) {
        ...
        if (this._response.domain === RESPONSE_DOMAINS.ROOM && this._response.room) {
          ...
          if (process && process.send)
            (<any>process).send({ data: this._response, target: 'others', action: 'response' })
          let io: SocketIO.Server = State[safe.get<'_io'>('State._io')];
